   ...
   YAHOO.extend(Alfresco.ConsoleTagManagement, Alfresco.component.Base,
   {
      ...
      _setupDataTable: function ConsoleTagManagement_setupDataTable()
      {
    	  ...
    	  var renderCellTagInfo = function ConsoleTagManagement_onReady_renderCellTagInfo(elCell, oRecord, oColumn, oData)
    	  {
    		  ...
    		  var messageDesc = '<span><a class="theme-color-1" target="_blank" href="' + Alfresco.constants.URL_PAGECONTEXT + 'repository#filter=tag|' + encodeURIComponent(tagName) + '&page=1"><b>' + $html(tagName) + '</b></a></span>';
    	  ...
    	  var renderCellTagOwner = function ConsoleTagManagement_onReady_renderCellTagOwner(elCell, oRecord, oColumn, oData)
    	  {
    		  ...
    		  var messageDesc = '<span><a class="theme-color-1" target="_blank" href="' + Alfresco.constants.URL_PAGECONTEXT + 'user/' + encodeURIComponent(tagOwner) + '/profile">' + $html(tagOwner) + '</a></span>';
          ...
          var renderCellActions = function ConsoleTagManagement_onReady_renderCellActions(elCell, oRecord, oColumn, oData)
          {
        	  ...
        	  actions += '<a id="' + me.id + '-delete-link-'+index+'" href="#" class="delete-tag" title="' + me.msg("title.deleteTag") + '"></a><span>';
      ...
      _performSearch: function ConsoleTagManagement__performSearch(args)
      {
    	 ...
    	 if (this.searchTerm == "" || this.searchTerm == "*") {
        	 ...
        	 this.widgets.dataSource.sendRequest("&searchTerm=" + encodeURIComponent(this.searchTerm),{
        	            success: successHandler,
        	            failure: failureHandler,
        	            scope: this
        	 });
      ...
      {
    	  var actionUrl = YAHOO.lang.substitute(Alfresco.constants.PROXY_URI+ "api/edit-tag/{store_type}/{store_id}/{tagName}",
					{
      ...
      _onActionDeleteConfirm: function ConsoleTagManagement_onActionDeleteConfirm(tagName)
      {
    	  Alfresco.util.Ajax.request(
          {
	          ...
	          url: YAHOO.lang.substitute(Alfresco.constants.PROXY_URI+ "api/delete-tag/{store_type}/{store_id}/{tagName}",
    		  {
	        	  store_type : "workspace",
	        	  store_id : "SpacesStore",
	        	  tagName: encodeURIComponent(tagName)
